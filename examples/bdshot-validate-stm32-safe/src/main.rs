...
use uf_dshot::proto::bitbang_bf_safe::stm32::{
    BidirCaptureConfig, BidirPortRuntimeConfig, DshotPortPin, InterruptHandler,
    Stm32BidirPinController,
};
use uf_dshot::{BidirTx, DecodeHint, DshotSpeed, OversamplingConfig};
#[cfg(feature = "defmt")]
...
const RX_COMPARE_PERCENT: u8 = 25;
const RX_SAMPLE_PERCENT: u8 = 115;
...
const STEP_THROTTLES: [u16; 1] = [150];
...
async fn hold_throttle(
    ...
    >,
    rx_sample_percent: u8,
    rx_compare_percent: u8,
    throttle: u16,
    duration: Duration,
    ...
    while Instant::now().saturating_duration_since(started) < duration {
        *frame_index = frame_index.wrapping_add(1);
        ...
        match esc.send_frame(BidirTx::throttle_clamped(throttle).encode()).await {
            Ok(()) => {
                counters.ok = counters.ok.wrapping_add(1);
                if *frame_index % TELEMETRY_LOG_EVERY_FRAMES == 0 {
                    info!(
                        "frame={} rx_sample_percent={} rx_compare_percent={} throttle={} tx=ok",
                        *frame_index, rx_sample_percent, rx_compare_percent, throttle
                    );
                }
            }
            Err(err) => {
                ...
                if *frame_index % TELEMETRY_LOG_EVERY_FRAMES == 0 {
                    info!(
                        "frame={} rx_sample_percent={} rx_compare_percent={} throttle={} tx_err={:?}",
                        *frame_index, rx_sample_percent, rx_compare_percent, throttle, err
                    );
                }
            }
        }
...
async fn app() {
    ...
    loop {
        ...
        unwrap!(esc.set_runtime_config(BidirPortRuntimeConfig {
            tx_timeout: Duration::from_millis(2),
            rx_timeout: Duration::from_millis(2),
            pacer_compare_percent: PACER_COMPARE_PERCENT,
            rx_compare_percent: RX_COMPARE_PERCENT,
        ...
        info!(
            "run summary rx_sample_percent={} rx_compare_percent={} total_frames={} ok={} io_err={}",
            RX_SAMPLE_PERCENT,
            RX_COMPARE_PERCENT,
            frame_index,
            counters.ok,
