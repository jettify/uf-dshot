...
use uf_dshot::{Command, DshotSpeed, DshotTx, EncodedFrame, WaveformTiming};
#[cfg(feature = "defmt")]
...
impl<'d> PwmDshotController<'d> {
    fn new(mut pwm: SimplePwm<'d, TIM1>, dma_ch: Peri<'d, DMA2_CH5>, speed: DshotSpeed) -> Self {
        ...
        Self {
            ...
            timing,
            motor_stop_cycles: frame_cycles(DshotTx::standard().command(Command::MotorStop), timing),
    ...
    async fn send_throttle(&mut self, throttle: u16) {
        write_frame_cycles(
            ...
            DshotTx::standard().throttle_clamped(throttle),
            self.timing,
        );
...
fn dshot_waveform_timing(max_duty: u32) -> WaveformTiming {
    let period_ticks = unwrap!(u16::try_from(max_duty));
    WaveformTiming {
        period_ticks,
...
async fn main(_spawner: Spawner) {
    ...
    let mut ticker = Ticker::every(Duration::from_micros(u64::from(
        ESC_SPEED.timing_hints().min_frame_period_us,
    )));
