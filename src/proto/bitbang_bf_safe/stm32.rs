...
use embassy_stm32_hal::dma::{
    ...
    InterruptHandler as EmbassyInterruptHandler, TransferOptions,
};
...
struct PreparedRxDmaConfig {
    request: embassy_stm32_hal::dma::Request,
    peri_addr: *mut u16,
    len: usize,
}
    ...
    D: DmaChannelInstance,
{
    pub fn new_ch1(
        timer: Peri<'d, T>,
        dma: Peri<'d, D>,
        dma_irq: impl Binding<D::Interrupt, EmbassyInterruptHandler<D>> + 'd,
        pins: [DshotPortPin<'d>; N],
        speed: DshotSpeed,
    ...
    where
        D: Dma<T, Ch1>,
    {
        Self::new_inner::<Ch1>(timer, dma, dma_irq, pins, speed)
    }
    ...
    where
        D: Dma<T, Ch2>,
    {
        Self::new_inner::<Ch2>(timer, dma, dma_irq, pins, speed)
    }
    ...
    where
        D: Dma<T, Ch3>,
    {
        Self::new_inner::<Ch3>(timer, dma, dma_irq, pins, speed)
    }
    ...
    where
        D: Dma<T, Ch4>,
    {
        Self::new_inner::<Ch4>(timer, dma, dma_irq, pins, speed)
    }
        ...
        D: Dma<T, C>,
    {
        ...
        let dma = DmaChannel::new(dma, dma_irq);
        let timer = Timer::new(timer);
        let runtime_cfg = PortRuntimeConfig::default();
        ...
        configure_pacer_timer(&this.timer, this.channel, this.tx_timer_cfg);
        Ok(this)
    }
    ...
    async fn run_tx(&mut self) -> Result<(), PortRuntimeError> {
        prepare_tx_start(&self.timer, self.channel);
        for pin in &mut self.pins {
            pin.enter_output_low();
        }

        let bsrr = self.pins[0].bsrr_ptr();
        let transfer = unsafe {
            self.dma.write(
                self.dma_request,
                &self.tx_words,
        ...
        start_pacer(&self.timer, self.channel);
        let result = with_timeout(self.runtime_cfg.tx_timeout, transfer).await;
        stop_pacer(&self.timer, self.channel);
    ...
    D: RawDmaChannel,
{
    ...
    #[cfg(feature = "defmt")]
    last_stop_gap_us: u32,
    #[cfg(feature = "defmt")]
    last_timer_switch_gap_us: u32,
    #[cfg(feature = "defmt")]
    last_dma_arm_gap_us: u32,
    #[cfg(feature = "defmt")]
    last_pacer_start_gap_us: u32,
    #[cfg(feature = "defmt")]
    last_rx_armed_us: u32,
    #[cfg(feature = "defmt")]
    ...
    D: RawDmaChannel,
{
        ...
        D: Dma<T, C>,
    {
        ...
        let this = Self {
            ...
            #[cfg(feature = "defmt")]
            tx_started_at: Instant::from_ticks(0),
            #[cfg(feature = "defmt")]
            last_tx_done_us: 0,
            #[cfg(feature = "defmt")]
            ...
            last_stop_gap_us: 0,
            #[cfg(feature = "defmt")]
            last_timer_switch_gap_us: 0,
            #[cfg(feature = "defmt")]
            last_dma_arm_gap_us: 0,
            #[cfg(feature = "defmt")]
            last_pacer_start_gap_us: 0,
            #[cfg(feature = "defmt")]
            last_rx_armed_us: 0,
            #[cfg(feature = "defmt")]
    ...
    pub async fn send_frame_and_receive(
        ...
        #[cfg(feature = "defmt")]
        {
            ...
            if self.capture_attempts <= 8 || self.capture_attempts % 256 == 0 {
                defmt::debug!(
                    "bdshot safe turnaround attempts={} rx_sample_percent={} rx_compare_percent={} tx_done_us={} release_gap_us={} stop_gap_us={} timer_switch_gap_us={} dma_arm_gap_us={} pacer_start_gap_us={} handoff_us={} capture_us={}",
                    self.capture_attempts,
                    self.runtime_cfg.rx_sample_percent,
                    self.runtime_cfg.rx_compare_percent,
                    self.last_tx_done_us,
                    self.last_release_gap_us,
                    self.last_stop_gap_us,
                    self.last_timer_switch_gap_us,
                    self.last_dma_arm_gap_us,
                    self.last_pacer_start_gap_us,
                    self.last_rx_armed_us.saturating_sub(self.last_tx_done_us),
            ...
            {
                defmt::info!(
                    "proto bdshot safe salvage attempts={} bf_start={} bf_end={} bf_bits={} bf_raw=0x{:06x}",
                    self.capture_attempts,
            ...
            if let Err(err) = outcome.frame {
                ...
                if count <= 8 || count % 256 == 0 {
                    ...
                    defmt::warn!(
                        "proto bdshot safe decode err attempts={} count={} rx_sample_percent={} rx_compare_percent={} err={} tx_done_irq_us={} rx_armed_irq_us={} handoff_us={} capture_us={} highs={} lows={} first_low={:?} edges={} first_edge={:?} last_edge={:?} hint_skip={} stats ok={} no_edge={} short={} invalid_frame={} invalid_gcr={} invalid_crc={} start_margin={} bf_start={} bf_end={} bf_bits={} bf_raw=0x{:06x} bf_raw_valid={} bf_payload=0x{:04x} bf_runs={},{},{},{},{},{},{},{},{},{},{},{} raw_head=0x{:04x},0x{:04x},0x{:04x},0x{:04x}",
                        self.capture_attempts,
                        count,
                        self.runtime_cfg.rx_sample_percent,
                        self.runtime_cfg.rx_compare_percent,
    ...
    async fn run_tx(&mut self) -> Result<(), PortRuntimeError> {
        ...
        loop {
            ...
            if dma_transfer_complete::<D>() {
                self.stop_dma_and_timer();
                self.pin.enter_input(self.capture_cfg.pull);
                return Ok(());
            }
    ...
    async fn run_tx_then_capture(&mut self) -> Result<(), PortRuntimeError> {
        ...
        #[cfg(feature = "defmt")]
        {
            ...
            self.last_tx_done_us = 0;
            self.last_release_gap_us = 0;
            self.last_stop_gap_us = 0;
            self.last_timer_switch_gap_us = 0;
            self.last_dma_arm_gap_us = 0;
            self.last_pacer_start_gap_us = 0;
            self.last_rx_armed_us = 0;
    ...
    unsafe fn dma_irq(ctx: *mut ()) {
        ...
        match this.irq_phase.load(Ordering::Acquire) {
            x if x == IrqPhase::TxActive as u8 => {
                #[cfg(feature = "defmt")]
                {
                    this.last_tx_done_us = Instant::now()
                        .saturating_duration_since(this.tx_started_at)
                        .as_micros() as u32;
                    let timer_switch_started_at = Instant::now();
                    switch_pacer_timer_config_fast(&this.timer, this.channel, this.rx_timer_cfg);
                    this.last_timer_switch_gap_us = Instant::now()
                        .saturating_duration_since(timer_switch_started_at)
                        .as_micros() as u32;
                    let dma_arm_started_at = Instant::now();
                    dma_start_prepared_read_no_reset::<D>(
                        this.rx_dma_cfg,
                        this.raw_samples.as_mut_ptr(),
                    );
                    this.last_dma_arm_gap_us = Instant::now()
                        .saturating_duration_since(dma_arm_started_at)
                        .as_micros() as u32;
                    let pacer_start_started_at = Instant::now();
                    this.irq_phase
                        .store(IrqPhase::RxActive as u8, Ordering::Release);
                    this.timer.set_cc_dma_enable_state(this.channel, true);
                    this.last_pacer_start_gap_us = Instant::now()
                        .saturating_duration_since(pacer_start_started_at)
                        .as_micros() as u32;
                    let release_started_at = Instant::now();
                    this.pin.enter_input(this.capture_cfg.pull);
                    this.last_release_gap_us = Instant::now()
                        .saturating_duration_since(release_started_at)
                        .as_micros() as u32;
                    this.last_stop_gap_us = 0;
                    this.last_rx_armed_us = Instant::now()
                        .saturating_duration_since(this.tx_started_at)
                        .as_micros() as u32;
                }
                #[cfg(not(feature = "defmt"))]
            ...
            x if x == IrqPhase::RxActive as u8 => {
                ...
                #[cfg(feature = "defmt")]
                {
                    let now_us = Instant::now()
                        .saturating_duration_since(this.tx_started_at)
                        .as_micros() as u32;
                    this.last_rx_capture_us = now_us.saturating_sub(this.last_rx_armed_us);
                }
                this.irq_phase
...
fn prepare_tx_start<T: GeneralInstance4Channel>(timer: &Timer<'_, T>, channel: Channel) {
    timer.stop();
    timer.reset();
    timer.set_cc_dma_enable_state(channel, false);
    timer.generate_update_event();
...
fn start_pacer<T: GeneralInstance4Channel>(timer: &Timer<'_, T>, channel: Channel) {
    timer.set_cc_dma_enable_state(channel, true);
    timer.start();
}

fn stop_pacer<T: GeneralInstance4Channel>(timer: &Timer<'_, T>, channel: Channel) {
    timer.stop();
    timer.set_cc_dma_enable_state(channel, false);
}
...
fn compute_rx_timer_config<T: GeneralInstance4Channel>(
    ...
    let base =
        speed.timing_hints().nominal_bitrate_hz * 5 * capture_cfg.oversampling.oversampling as u32
            / 4;
    let hz = base.saturating_mul(runtime_cfg.rx_sample_percent.clamp(1, 200) as u32) / 100;
    compute_pacer_timer_config(timer, hz, runtime_cfg.rx_compare_percent)
}
